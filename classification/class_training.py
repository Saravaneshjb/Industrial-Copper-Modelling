...
class ClassificationTrainingPipeline:
    ...
    def preprocess(self):
        ...
        ## Filtering the records with status as WON,LOST
        copper_df=copper_df[(copper_df['status']=='Won') | (copper_df['status']=='Lost')]
        ## Resetting the index in the dataframe. 
        copper_df.reset_index(drop=True, inplace=True)
        
        # 7. Feature Encoding
        # for col in ['status', 'item type']:
        for col in ['item type']:
            le = LabelEncoder()
            copper_df[col] = le.fit_transform(copper_df[col])
            self.label_encoders[col] = le
        
        # 8. Deleting records with negative values in selling price and quantity tons  
        copper_df = copper_df[(copper_df['selling_price'] >= 0) & (copper_df['quantity tons'] >= 0)]
        
        # 9. Outlier Handling - Cap Method
        for col in ['thickness', 'width', 'quantity tons','selling_price']:
        ...
        copper_df[['thickness', 'width', 'quantity tons','selling_price']] = self.scaler.fit_transform(copper_df[['thickness', 'width', 'quantity tons','selling_price']])
    ...
    def build_model(self):
        ...
        X = self.preprocessed_df.drop('status', axis=1)
        y = self.preprocessed_df['status']
...
if __name__ == "__main__":
    train_pipeline = ClassificationTrainingPipeline('D:\\Saravanesh Personal\\Guvi\\Capstone Projects\\Industrial Copper Mining\\Copper_Set.xlsx')
    ...
    pickle_path = 'D:\\Saravanesh Personal\\Guvi\\Capstone Projects\\Industrial Copper Mining\\classification\\class_pickle_files'
